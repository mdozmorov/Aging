...
library(knitr) 
opts_chunk$set(cache.path='cache/', fig.path='img/', cache=F, tidy=T, fig.keep='high', dpi=200,  echo=F, autodep=T, width=200, warning=FALSE, comment=NA)
...
Regulatory similarity among datasets using emryonic cell lines
===

All markers, excluding pos/neg
---
```{r}
...
For each pair of aDMRs and aGENs, we calculated epigenomic similarity coefficient (Spearman correlation coefficient between corresponding epigenomic enrichment profiles). The matrix of epigenomic similarity coefficients is visualized as a heatmap, with blue/red gradient correspontind to low/high epigenomic similarity between the profiles.
...
Histone modification marks enrichment overview
---
```{r fig.height=5.5}
#dev.off()
...
Transcription factor binding sites enrichment overview
---
```{r fig.height=5.5}
#dev.off()
...
Chromatin states enrichment overview
---
```{r fig.height=5.5}
#dev.off()
...
```{r defineClusters2, echo=FALSE, dependson="posnegHeat"}
par(oma=c(0, 0, 0, 0), mar=c(5.1, 4.1, 4.1,25.1), cex=0.5)
# Plot the dendrogram only, limit y axis. attr(h$colDendrogram, "height") has the maximum height of the dendrogram.
plot(h$colDendrogram, horiz=T) 
# Cut the dentrogram into separate clusters. Tweak the height
...
Regulatory similarity among datasets using all cell lines
===
```{r fig.height=6}
